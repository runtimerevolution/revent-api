...
resource "aws_alb_target_group" "revent_app_tg" {
  name        = "revent-app-tg"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_alb_listener" "revent_app_alb_listener" {
  load_balancer_arn = aws_lb.revent_app_lb.id
  port              = "80"
  protocol          = "HTTP"
  depends_on        = [aws_alb_target_group.revent_app_tg]
